...
export default function FeedPage() {
  ...
  const [filterType, setFilterType] = useState<
    'following_and_my_posts' | 'all' | 'following' | 'disease' | 'my_posts' | 'not_following'
    ...
    loadFn: useCallback(
      async (skip, limit) => {
        ...
        if (activeTab === 'saved') {
          ...
          const fetchedPosts = await getSavedPosts(skip, limit, savedSortBy, savedSortOrder, token);
          return {
            items: fetchedPosts,
          };
        }
  ...
  const handleFilterChange = (
    newFilterType: 'all' | 'following' | 'disease' | 'my_posts' | 'not_following'
  ) => {
  ...
  const getDiseaseName = (diseaseId: number): string => {
    ...
    const translation = userDisease.disease.translations?.find(t => t.language_code === locale);
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="mt-4">
                  <div className="flex items-center space-x-4">
                    ...
                    >
                      ...
                      <option value="post_created_at">
                        {t('savedSortBy.postDate') || '投稿日'}
                      </option>
                    </select>
                    <select
          ...
          {!isAuthenticated && (
            <div className="mb-6 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
              <p className="text-blue-800 dark:text-blue-200">{t('loginPrompt')}</p>
            </div>
          )}
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-12 text-center">
                ...
                <p className="mt-2 text-gray-500 dark:text-gray-400">
                  {activeTab === 'saved'
                    ...
                    : filterType === 'following_and_my_posts'
                    ? t('noFollowingAndMyPostsMessage') ||
                      'フォローしているユーザーや自分の投稿がまだありません'
                    : filterType === 'disease'
                    ? t('noDiseasePostsMessage', {
                        diseaseName: selectedDiseaseId ? getDiseaseName(selectedDiseaseId) : '',
                      })
                    : filterType === 'my_posts'
                    ? t('noMyPostsMessage')
                    : t('noPostsMessage')}
