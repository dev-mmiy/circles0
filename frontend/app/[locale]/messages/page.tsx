...
import { getConversations, deleteConversation, Conversation } from '@/lib/api/messages';
...
export default function MessagesPage() {
  ...
  const {
    ...
    loadFn: async (skip, limit) => {
      ...
      const conversations = conversationsResponse.conversations.map(conv => ({
        ...conv,
  ...
  const conversations = allItems.filter(
    (item): item is Conversation & { type: 'conversation' } => item.type === 'conversation'
  );
  const groups = allItems.filter(
    (item): item is Group & { type: 'group' } => item.type === 'group'
  );
  ...
  const handleNewMessage = useCallback(
    (messageEvent: MessageEvent) => {
      ...
      const itemIndex = conversationsRef.current.findIndex(
        item =>
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {error && (
            <div className="mb-6">
              <ErrorDisplay error={error} onRetry={retry} showDetails={true} />
            </div>
          )}
          ...
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div className="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
              <div className="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                {modalTab === 'group' &&
                  searchQuery.trim() &&
                  modalGroupSearchResults.length > 0 && (
                    <div className="max-h-96 overflow-y-auto border-t border-gray-200 dark:border-gray-700">
                      <div className="py-2">
                        <div className="px-4 py-2 text-sm font-semibold text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                          {t('searchResults')}
