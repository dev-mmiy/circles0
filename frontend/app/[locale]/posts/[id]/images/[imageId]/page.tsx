...
export default function PostImageViewerPage() {
  ...
  const getInitialIndex = useCallback(() => {
    ...
    const index = post.images.findIndex((img) => img.id === imageId);
    return index >= 0 ? index : 0;
  }, [post, imageId]);

  // Convert post images to ImageViewerImage format
  const getImageViewerImages = useCallback((): ImageViewerImage[] => {
    if (!post?.images) return [];
    return post.images.map((img) => ({
      id: img.id,
      image_url: img.image_url,
      alt: `Post image ${img.display_order}`,
    }));
  }, [post]);

  const handleClose = useCallback(() => {
    // Use router.back() to return to the previous page (e.g., /feed)
    router.back();
  }, [router]);
      ...
      <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-50">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-12 text-center max-w-md mx-4">
          ...
          <p className="mt-2 text-gray-500 dark:text-gray-400">
            {error || 'The post or image you are looking for does not exist.'}
          </p>
          <div className="mt-6">
            <button
  ...
  const images = getImageViewerImages();
