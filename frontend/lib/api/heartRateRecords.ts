  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response
        .json()
        .catch(() => ({ detail: 'Failed to fetch heart rate records' }));
...
export async function getHeartRateRecord(
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response
        .json()
        .catch(() => ({ detail: 'Failed to fetch heart rate record' }));
      throw new Error(errorData.detail || `HTTP error! status: ${response.status}`);
    }
...
export async function createHeartRateRecord(
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response
        .json()
        .catch(() => ({ detail: 'Failed to create heart rate record' }));
      throw new Error(errorData.detail || `HTTP error! status: ${response.status}`);
    }
...
export async function updateHeartRateRecord(
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response
        .json()
        .catch(() => ({ detail: 'Failed to update heart rate record' }));
      throw new Error(errorData.detail || `HTTP error! status: ${response.status}`);
    }
 ...
 */
export async function deleteHeartRateRecord(recordId: string, accessToken: string): Promise<void> {
  const url = `${apiClient}/api/v1/heart-rate-records/${recordId}`;
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response
        .json()
        .catch(() => ({ detail: 'Failed to delete heart rate record' }));
      throw new Error(errorData.detail || `HTTP error! status: ${response.status}`);
    }
  } catch (error: any) {
