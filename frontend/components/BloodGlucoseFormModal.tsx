...
interface BloodGlucoseFormModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function BloodGlucoseFormModal({
  isOpen,
  onClose,
  onRecordCreated,
  editingRecord,
}: BloodGlucoseFormModalProps) {
          ...
          >
            <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">
                {editingRecord ? tDaily('editVital') || 'バイタルを編集' : tDaily('addBloodGlucose') || '血糖値を記録'}
              </h2>
              <button
                onClick={onClose}
            ...
            <div className="p-4 sm:p-6">
              <BloodGlucoseForm onRecordCreated={handleRecordCreated} editingRecord={editingRecord} />
            </div>
          </div>
        </div>
      </div>
