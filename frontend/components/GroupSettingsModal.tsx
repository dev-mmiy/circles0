...
export default function GroupSettingsModal({ group, isOpen, onClose, onUpdate }: GroupSettingsModalProps) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                ...
                <div className="flex border-b">
                    <button
                        className={`flex-1 py-3 px-4 text-sm font-medium ${activeTab === 'general' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                        onClick={() => setActiveTab('general')}
                    >
                        {t('general')}
                    </button>
                    <button
                        className={`flex-1 py-3 px-4 text-sm font-medium ${activeTab === 'members' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                        onClick={() => setActiveTab('members')}
                    >
                        {t('members')}
                    </button>
                    <button
                        className={`flex-1 py-3 px-4 text-sm font-medium ${activeTab === 'danger' ? 'border-b-2 border-red-500 text-red-600' : 'text-gray-500 hover:text-gray-700'}`}
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    ...
                    {activeTab === 'general' && (
                        <div className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">{t('groupDescription')}</label>
                                <textarea
                                    value={description}
                        ...
                        <div className="space-y-6">
                            ...
                            {isAdmin && (
                                <div className="space-y-2">
                                    <label className="block text-sm font-medium text-gray-700">{t('addMembers')}</label>
                            ...
                            {/* Member List */}
                            <div>
                                <h3 className="font-medium mb-2">{t('members')} ({group.members.length})</h3>
                                <div className="space-y-2">
                                        ...
                                        <div key={member.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    {member.user?.username && <div className="text-xs text-gray-500">@{member.user.username}</div>}
                                                </div>
                                            </div>
                                            ...
                                            {isAdmin && member.user_id !== currentUser?.id && (
                                                <div className="flex items-center gap-2">
                                                    <button
                                                        ...
                                                        className={`p-1 rounded hover:bg-gray-200 ${member.is_admin ? 'text-blue-600' : 'text-gray-400'}`}
