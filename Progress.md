...
- [x] **フロントエンド実装** ✅ 完了（2025-11-15）
  ...
  - [x] ダークモード対応 ✅ 完了（2025-11-27）
    - [x] メッセージ一覧ページのダークモード対応
    - [x] 個別会話ページのダークモード対応
    - [x] 検索機能、モーダル、ボタンなどのUI要素のダークモード対応
    - [x] グループチャットページのダークモード対応
    - [x] グループ一覧ページのダークモード対応
    - [x] グループ作成ページのダークモード対応
    - [x] プロフィールページのダークモード対応
    - [x] 投稿詳細ページのダークモード対応
    - [x] ブロックページのダークモード対応
    - [x] 通知ドロップダウンのダークモード対応
    - [x] 疾患検索結果のダークモード対応（検索履歴、オートコンプリート、エラーメッセージ含む）
    - [x] ユーザー検索結果リストのダークモード対応（検索履歴、高度な検索オプション、エラーメッセージ含む）
- [x] **メッセージとグループの統合** ✅ 完了（2025-11-27）
  - [x] メッセージページの検索機能拡張 ✅ 完了
    - [x] 会話検索をユーザー検索とグループ検索に拡張 ✅ 完了
    - [x] 検索結果にユーザーとグループの両方を表示 ✅ 完了
    - [x] 検索結果から直接会話/グループチャットを開始 ✅ 完了
  - [x] グループ作成ボタンの追加 ✅ 完了
    - [x] メッセージページ上部に「グループを作成」ボタンを追加 ✅ 完了
    - [x] クリックでグループ作成ページに遷移 ✅ 完了
  - [x] 新しいメッセージモーダルの拡張 ✅ 完了
    - [x] ユーザー/グループ選択タブまたはラジオボタンを追加 ✅ 完了
    - [x] ユーザー選択時：既存のユーザー検索機能を使用 ✅ 完了
    - [x] グループ選択時：グループ検索とグループ作成オプションを表示 ✅ 完了
    - [x] グループ選択時はグループ一覧を表示し、選択可能にする ✅ 完了
...
- [ ] **食事記録**
  ...
  - [ ] 食事写真の添付
  - [ ] 食事メモ・感想
  - [ ] **カレンダー表示** ⏳ 実装予定（バイタル記録と同様の実装計画を参照）
  - [ ] 食事履歴の検索・フィルタリング
  - [ ] 食事パターンの分析（グラフ表示）
- [ ] **食材・料理管理**
...
- [ ] **栄養成分計算・管理**
  ...
  - [ ] 栄養成分の警告・アラート
    ...
    - [ ] バランスの悪い食事パターンの検出
- [ ] **バイタル記録**
  - [x] 血圧・心拍数記録 ✅ 完了（2025-12-30）
  - [x] 体温記録 ✅ 完了（2025-12-30）
  - [x] 体重・体脂肪率記録 ✅ 完了（2025-12-30）
  - [x] 血糖値記録 ✅ 完了（2025-12-30）
  - [x] 血中酸素濃度（SpO2）記録 ✅ 完了（2025-12-30）
  - [x] 日々の記録ページ（リスト表示） ✅ 完了（2025-12-30）
  - [ ] **カレンダー表示** ⏳ 実装予定
    - [ ] 月間カレンダー表示
      - [ ] `date-fns`を使用したカレンダーグリッド生成
      - [ ] 各日に記録があるかを視覚的に表示（背景色・ドット表示）
      - [ ] 記録の種類ごとの色分け表示
        - 血圧・心拍数: 赤系
        - 体温: 青系
        - 体重・体脂肪: 緑系
        ...
        - SpO2: ピンク系
      - [ ] 月の切り替え機能（前月/次月）
      - [ ] 日をクリックするとその日の記録を表示（モーダルまたは詳細表示）
      - [ ] 記録がない日の視覚的区別
    - [ ] データ構造
      - [ ] 日付をキーとして記録をグループ化（`yyyy-MM-dd`形式）
      - [ ] カレンダー表示用のデータ変換ロジック
      - [ ] 記録の種類ごとの集計・表示
    - [ ] UI/UX
      - [ ] カレンダーグリッドのレスポンシブ対応
      - [ ] ダークモード対応
      - [ ] 曜日ヘッダーの表示（日、月、火、水、木、金、土）
      - [ ] 今日の日付のハイライト表示
      - [ ] 記録数に応じた視覚的フィードバック（複数記録がある日は複数のドット表示）
  - [ ] グラフ表示（将来実装予定）
...
### 優先度: 高
1. **多言語対応の拡充** ✅ 主要機能完了
   - ✅ 基盤となる35コンポーネント/ページの翻訳完了 (約381キー、26名前空間)
...
3. **メッセージとグループの統合** ✅ ほぼ完了（2025-11-27）
   - **Phase 1: バックエンドAPIの確認・拡張**
     - [ ] グループ検索APIの確認（既存の`searchGroups` APIの動作確認）
     - [ ] 必要に応じてAPIの拡張（ユーザーとグループを統合して検索するエンドポイントの追加を検討）
   - **Phase 2: フロントエンド実装**
     - [x] メッセージページの検索機能拡張 ✅ 完了（2025-11-27）
       - [x] 会話検索をユーザー検索とグループ検索に拡張
       - [x] 検索結果にユーザーとグループの両方を表示
       - [x] 検索結果から直接会話/グループチャットを開始
     - [x] グループ作成ボタンの追加 ✅ 完了（2025-11-27）
       - [x] メッセージページの上部に「グループを作成」ボタンを追加
       - [x] クリックでグループ作成ページ（`/groups/new`）に遷移
     - [x] 新しいメッセージモーダルの拡張 ✅ 完了（2025-11-27）
       - [x] ユーザー/グループ選択タブを追加
       - [x] ユーザー選択時：既存のユーザー検索機能を使用
       - [x] グループ選択時：グループ検索とグループ作成オプションを表示
       - [x] グループ選択時はグループ一覧を表示し、選択可能にする
   - **Phase 3: 統合とテスト**
     - [x] 会話リストにグループも表示（既存の会話とグループを統合表示） ✅ 完了（2025-11-27）
     - [x] 翻訳キーの追加（日本語・英語） ✅ 完了（2025-11-27）
     - [x] 動作確認とテスト ✅ 完了（2025-11-27）

### 優先度: 中
1. **パフォーマンス最適化の確認** ⏳ 進行中
   - [x] get_feedのN+1クエリ問題の修正 ✅ 完了（2025-11-21）
   - [x] get_conversationsのN+1クエリ問題の修正 ✅ 完了（2025-11-21）
