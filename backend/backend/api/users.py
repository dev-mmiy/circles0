...
async def create_user(user_data: UserCreate, db: Session = Depends(get_db)):
    ...
    existing_user = (
        db.query(User)
        .filter((User.email == user_data.email) | (User.idp_id == user_data.idp_id))
        .first()
    )
...
async def create_user_preference(
    user_id: UUID, preference_data: UserPreferenceCreate, db: Session = Depends(get_db)
):
    ...
    if existing_preference:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Preference already exists"
        )
...
@router.put(
    "/{user_id}/preferences/{preference_id}", response_model=UserPreferenceResponse
)
