...
def upgrade() -> None:
    ...
    op.create_table('name_display_orders',
    ...
    op.create_table('locale_name_formats',
        sa.Column('id', sa.Integer(), primary_key=True),
        sa.Column('locale', sa.String(length=10), nullable=False, unique=True),
        sa.Column('default_order_code', sa.String(length=20), sa.ForeignKey('name_display_orders.order_code')),
        sa.Column('is_active', sa.Boolean(), default=True),
        sa.Column('created_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'))
    )
